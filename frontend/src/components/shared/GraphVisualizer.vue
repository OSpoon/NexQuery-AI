...
watch(
  () => [props.mermaidString, props.activeNodeName],
  () => {
    renderGraph()
  },
)
...
<template>
  <div
    class="mermaid-viewer relative w-full bg-muted/5 rounded-(--radius) border border-border/60 shadow-sm overflow-hidden group"
  >
    <!-- Controls Overlay -->
    <div
      class="absolute right-2 top-2 z-10 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200"
    >
      <div
        class="mb-1 text-[10px] font-mono bg-background/80 backdrop-blur-sm px-1.5 py-0.5 rounded border shadow-sm text-center tabular-nums"
      >
        {{ zoomPercentage }}
      </div>
      <Button
        variant="secondary"
        size="icon"
        class="h-8 w-8 rounded-md bg-background/80 backdrop-blur-sm border shadow-sm"
        title="Reset View"
        @click="resetView"
      >
