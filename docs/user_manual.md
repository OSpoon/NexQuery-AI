...
11. [工作流审批 (Workflow Approval)](#11-工作流审批)
...
## 11. 工作流审批 (Workflow Approval)

为了确保核心业务数据的安全性，NexQuery AI 集成了 **Flowable** 工作流引擎，支持对高危 SQL 操作进行强制人工审批。

### 11.1 核心流程：高危 SQL 审批
1.  **触发阶段**: 当用户在 SQL 编辑器或 AI 助手中尝试执行包含高危关键字（如 `DELETE`, `DROP`, `ALTER`）的 SQL 时，系统会自动拦截。
2.  **提交申请**: 用户需要填写申请理由，随后 SQL 会进入待审批状态。
3.  **人工审批**:
    *   审批人通过 **Workflows -> History** 或待办任务列表查看申请。
    *   审批人可以查看完整的 SQL、申请理由及当前环境，点击 **Approve** (批准) 或 **Reject** (拒绝)。
4.  **执行与通知**:
    *   **批准后**: 任务自动执行，结果将通过邮件通知发起人。
    *   **拒绝后**: 流程结束，发起人收到拒绝建议邮件。

### 11.2 工作流设计器 (Designer)
管理员可以在 **Workflow -> Designer** 中自定义审批逻辑：
*   **拖拽式编辑**: 基于 BPMN 2.0 标准，支持节点、连线与网关。
*   **精简配置**:
    *   **类型选择**: 专为 **SQL Approval** 优化。
    *   **优先级**: 默认为高优先级（High）。
    *   **条件表达式**: 通过下拉框直接选择 `已批准` 或 `已拒绝` 逻辑。
*   **Webhook 通知**: 支持通过 HTTP 请求（Webhook）对接外部通知系统。
    *   **动态变量**: 支持通过 URL 参数传递变量，例如 `${initiatorEmail}`。

### 11.3 管理与审计
*   **删除检查**: 系统在删除工作流定义前会强制检查是否有“进行中”的任务，确保流程完整性。
*   **版本管理**: 所有部署的工作流均带有版本号（v1, v2...），确保历史任务的可追溯性。

*核心控制器*: [`WorkflowController`](../backend/app/controllers/workflow_controller.ts)
*核心服务*: [`WorkflowService`](../backend/app/services/workflow_service.ts)
