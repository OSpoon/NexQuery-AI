...
*   **查询能力**: 支持通过自然语言生成 Lucene 语法（如 `status:200 AND @timestamp:[now-1h TO now]`）。
*   **探索工具**: AI 会自动列出所有 Index，并能分析 Mapping 和字段分布 (Field Stats)，帮助您快速定位数据。
...
*   **用户校正 (Correction)**: 如果 AI 生成的 SQL 有误，用户可以提供正确的 SQL。
*   **自动学习 (Adopt & Learn)**: 管理员审核反馈后，点击 **Promote**。系统会将优质案例**自动同步到知识库**。如果用户点赞且 AI 的置信度较高，系统甚至能实现**自动学习 (Autonomous Learning)**，无需人工干预即可在下次遇到类似问题时生成更准确的代码。
*   **双模知识库**: 知识库现已支持 **SQL** 和 **Lucene** 两种类型。AI 在检索时会自动根据当前数据源类型加载对应的知识，互不干扰。
